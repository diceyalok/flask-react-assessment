...
const Dashboard: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditingId) {
        await axios.patch(`${API_URL}/${isEditingId}`, payload, { headers: getHeaders() });
      } else {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div style={{ background: '#f4f4f4', padding: '20px', borderRadius: '8px', marginBottom: '20px' }}>
        <h3 className="font-bold mb-2">{isEditingId ? 'Edit Task' : 'New Task'}</h3>
        <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '10px' }}>
          <input 
            type="text" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} 
            required style={{ padding: '8px', flex: 1 }}
          />
          <input 
      ...
      <div>
        ...
        {tasks.map((task) => (
          <div key={task.id} style={{ borderBottom: '1px solid #eee', padding: '15px', display: 'flex', justifyContent: 'space-between' }}>
            <div>
              <strong>{task.title}</strong>
              <div style={{ color: '#666' }}>{task.description}</div>
            </div>
            <div>
              <button onClick={() => handleEdit(task)} style={{ marginRight: '10px', color: 'blue', cursor: 'pointer' }}>Edit</button>
              <button onClick={() => handleDelete(task.id)} style={{ color: 'red', cursor: 'pointer' }}>Delete</button>
            </div>
          </div>
        ))}
      </div>
