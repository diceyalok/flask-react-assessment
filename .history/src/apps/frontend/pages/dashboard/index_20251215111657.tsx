...
const Dashboard: React.FC = () => {
  ...
  // --- CONFIGURATION ---
  // We use the full URL with /api to ensure we hit the correct backend route
  const API_URL =
    'http://localhost:8080/api/accounts/693f99e5ca59562f35431332/tasks';
  // 2. Function to load tasks
  const fetchTasks = async () => {
    try {
      const response = await axios.get(API_URL);
      // Ensure we are setting an array (handles if backend returns wrapped data)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // The backend expects "account_id" sometimes, so we add a dummy one just in case
      const payload = {
        title,
        description,
        account_id: 'user_123', // Dummy ID to satisfy potential backend rules
      };
