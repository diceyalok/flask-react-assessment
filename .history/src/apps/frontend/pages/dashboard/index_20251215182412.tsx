...
interface Task {
  id: string; // Changed to string as Mongo IDs are strings
  title: string;
  description: string;
...
const Dashboard: React.FC = () => {
  ...
  // --- FIX #1: Correctly extract the token string from the JSON object ---
  const getHeaders = () => {
    ...
    if (storedValue) {
      try {
        // We must parse it because it's saved as '{"token":"...", "accountId":"..."}'
  ...
  const fetchTasks = async () => {
    try {
      ...
      // Backend returns { tasks: [...], total: ... } based on your Service code
      // We check where the array is hiding
      const data =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditingId) {
        // --- FIX #2: Backend expects PATCH, not PUT ---
